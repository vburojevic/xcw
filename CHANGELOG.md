...
### Changed
- `xcw examples` now covers more commands (config/pick/completion/summary/update/version/log-schema/handoff/ui/clear).
...
### Changed
- `xcw help --json` now documents `watch` and includes `--dry-run-json` examples for `tail`/`query`/`watch`.
...
### Fixed
- Avoid treating benign `os.ErrClosed` errors from stderr scanners as fatal in `xcw query` and log streaming.
